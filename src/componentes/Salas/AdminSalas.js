...
import "../../styles/Salas/AdminSalas.css";
      ...
      <div className="encabezado-reserva">
        ...
        <div className="fecha-hora">
          ...
          <div className="hora-reserva">
            <strong>Hora de Inicio:</strong> {reserva.hora_inicio}
            <br /> <strong>Hora de Fin:</strong> {reserva.hora_fin}
          </div>
        </div>
        <div className="reservado-por">
          <strong>Reservado por:</strong>
          <br /> {reserva.reservado_por}
        </div>
        <div className="asistentes">
          <h4>Asistentes:</h4>
          <ul>
            {asistentes.length > 0 ? (
              asistentes.map((nombre, indice) => <li key={indice}>{nombre}</li>)
            ) : (
...
const ListaReservas = () => {
  ...
  const manejarEliminar = async (id) => {
    try {
      ...
      setReservas((reservasPrevias) =>
        reservasPrevias.filter((reserva) => reserva.id !== id)
      );
  ...
  const guardarReservaEditada = async (evento) => {
    ...
    const empleadosAsistentesIds = Array.from(
      datosFormulario.getAll("empleados_asistentes")
    );
  ...
  return (
    <div className="contenedor-reservas">
      ...
      {modoEdicion && reservaParaEditar && (
        <form
          onSubmit={guardarReservaEditada}
          className="formulario-editar-reserva"
        >
          ...
          <div>
            ...
            <select
              ...
              id="sala"
              required
              defaultValue={reservaParaEditar.sala}
            >
              {salas.map((sala) => (
      ...
      {modoCreacion && (
        <form
          onSubmit={manejarCreacionReserva}
          className="formulario-editar-reserva"
        >
